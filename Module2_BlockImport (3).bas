...
'       Indonesia:     "AGENCY n" -> ACn      (AGENCY 1 -> AC1)
'       Other markets: "AGENCY n" -> AC(n-1)  (client numbering is one lower,
'                      e.g. AGENCY 2 -> AC1, AGENCY 10 -> AC9;
'                      AGENCY 1 is flagged as unknown account)
'       No number after AGENCY -> flagged as unknown account
...
Sub RunAllBlockImport()
    ...
    For r = 2 To rawLast
        ...
        If isin <> "" Then
            ...
            acctVal = AccountFromDesc(CStr(wsRaw.Cells(r, RC_DESC).Value), mktCode)
...
' Helpers
'---------------------------------------------------------
Private Function AccountFromDesc(ByVal desc As String, ByVal mktCode As String) As String
    ...
    If prevTok = "AGENCY" And IsDigits(lastTok) Then
        If mktCode = "ID" Then
            ' Indonesia: same number on both sides (AGENCY 1 -> AC1)
            If CLng(lastTok) >= 1 Then AccountFromDesc = "AC" & CLng(lastTok)
        Else
            ' Other markets: client number is one lower (AGENCY 2 -> AC1)
